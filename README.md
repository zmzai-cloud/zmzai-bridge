...
- 暴露**内部调度 API**，让 `zmzai-relay` / `zmzai-sandbox` 等云端 Agent 把工具请求下发到用户本机；
- 关联客户端回传的 `tool_result`，作为 HTTP 响应返回给发起请求的 Agent。

```
┌──────────────┐   出站 WS /bridge    ┌────────────────┐   内部 HTTP /v1/*   ┌────────────────┐
│ zmzai-client │ ───────────────────► │  zmzai-bridge  │ ◄───────────────── │  relay / sandbox│
...
│  └─ index.ts               # 启动入口
├─ sdk/bridge-sdk.ts         # 供 relay/sandbox vendored 的轻量调用客户端
├─ scripts/
...
`zmzai-relay`、`zmzai-sandbox` 的后端在需要「把工具执行落到用户本机」时，调用本服务的调度 API：

1. 把 `sdk/bridge-sdk.ts` 整文件复制到调用方仓库（无额外依赖）。
2. 用内部 Token 构造 `new BridgeClientSdk("https://b.zmzai.cloud", BRIDGE_TOKEN)`。
3. 在 Agent 会话里拿到目标 `sessionId`（客户端连上后由 bridge 分配，relay 需在用户登录时把 `clientId ↔ 用户` 建立映射，并通过 `GET /v1/sessions` 或客户端主动上报获知用户的 `sessionId`）。
4. 调用 `bridge.dispatchToSession(sessionId, "fs.read", { path })` 等。
...
- **握手**：客户端用 `CLIENT_SECRET` 对 `clientId:ts` 做 HMAC；桥接校验签名 + 时间戳（拒绝 `>HELLO_MAX_AGE_MS` 的重放）。
...
- [ ] 客户端主动上报 `sessionId` → relay 用户会话的绑定（WebSocket 或轮询）。
